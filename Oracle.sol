...
contract TransactionPrice is Chainlinked {
  bytes public txHaash;
  ...
  bytes32 constant JOB_ID = bytes32("3116f19a4b00434ba28b4a5cec349474");
  ...
  function verifyTx(string url) public {
    ChainlinkLib.Run memory run = newRun(JOB_ID, this, "report(bytes32,bytes32)");
    run.add("url", url);
  ...
  function report(bytes32 _externalId, bytes32 hash)
    public
  ...
  */
  function isFinalized(bytes _query, uint _timeout) public view returns (bool) {
      return txHash == bytesToBytes32(_query, 0);
  }
